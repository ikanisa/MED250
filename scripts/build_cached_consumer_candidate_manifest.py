#!/usr/bin/env python3
"""Build an exact-ASIN consumer manifest from the existing Bing cache."""
...
DEFAULT_OUTPUT = (
    pipeline.REPO_ROOT
    / "data/product-images/cached-exact-consumer-candidates.json"
)
...
def build_manifest(dataset: Path, cache_dir: Path) -> list[dict[str, Any]]:
    ...
    for product in pipeline.load_products(dataset):
        if product.group != "consumer" or not product.asin:
            continue
        exact_asin = product.asin.lower()
        for item in cached_bing_rows(cache_dir, product):
            ...
            evidence = " ".join([title, page_url, image_url]).lower()
            key = (product.id, image_url)
            if (
                not page_url
                or not image_url
                or exact_asin not in evidence
