...
export function assessDeploymentEvidence({ origin, mode, records }) {
  ...
  if (home) {
    ...
    if (mode === "live" && home.headers["x-robots-tag"]) errors.push("/: live custom domain is unexpectedly blocked from indexing");
  }
...
function parseArguments(values) {
  ...
  if (!new Set(["preview", "live"]).has(parsed.mode)) throw new Error("--mode must be preview or live.");
  return parsed;
}
...
export function validateDeploymentOrigin(value, mode) {
  ...
  if (mode === "live" && !new Set(["med250.rw", "www.med250.rw"]).has(url.hostname)) {
