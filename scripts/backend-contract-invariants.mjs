export const expectedBackendContractVersion = "2026-07-15.2";
...
export function assessBackendContract(contract) {
  ...
  requireInvariant(contract?.marketplace_catalogue?.minimum_taxonomy_pair_count >= 80, "A marketplace subcategory has fewer than 80 products.");
  ...
  requireInvariant(contract?.marketplace_moderation?.approved_without_evidence_count === 0, "An approved marketplace product lacks review evidence.");
  ...
  requireInvariant(contract?.api_surface?.expected_function_count === 28, "MED+250 function allowlist count drifted.");
  ...
  requireInvariant(contract?.table_surface?.expected_table_count === 21, "MED+250 table allowlist count drifted.");
