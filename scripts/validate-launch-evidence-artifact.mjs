  ...
  if (strict || artifact?.status === "complete") {
    switch (artifact?.evidence_type) {
      ...
      case "domain_verification":
        ...
        for (const hostname of ["med250.rw", "www.med250.rw"]) if (!Array.isArray(artifact?.hostnames) || !artifact.hostnames.includes(hostname)) errors.push(`domain verification must include ${hostname}`);
