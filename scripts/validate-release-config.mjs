...
if (!existsSync(wranglerPath)) {
  ...
  try {
    ...
    if (liveRequired) {
      ...
      if (workerName !== "med250-marketplace") errors.push("The production Worker must be named med250-marketplace.");
      ...
      if (!routePatterns.includes("med250.rw/*") || !routePatterns.includes("www.med250.rw/*")) {
        errors.push("The production Worker must route both med250.rw and www.med250.rw.");
      }
    }
  } catch {
...
if (!new Set(["preview", "live"]).has(mode)) {
  errors.push("NEXT_PUBLIC_MARKETPLACE_MODE must be preview or live.");
}

if (!new Set(["preview", "live"]).has(workerMode)) {
  errors.push("wrangler.jsonc vars.MED250_RELEASE_MODE must be preview or live.");
...
if (mode !== "live") warnings.push("Marketplace mode is preview, so customer ordering remains disabled.");
if (!siteUrl) warnings.push("NEXT_PUBLIC_SITE_URL is not explicitly configured; metadata uses the med250.rw default.");
