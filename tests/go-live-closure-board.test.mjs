...
test("builds an owner closure board without promoting pending gates", () => {
  ...
  assert.equal(board.summary.legacy_redirect_passed_probes, 0);
  assert.equal(board.summary.legacy_redirect_required_probes, 5);
  assert.equal(board.summary.legacy_redirect_valid, false);
  ...
  assert.equal(board.prerequisites.legacy_domain_redirect.valid, false);
  ...
  assert.match(board.prerequisites.legacy_domain_redirect.blocker, /redirect-only contract/);
