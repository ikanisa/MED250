...
test("builds approval packets only for evidence-complete unapproved launch gates", () => {
  ...
  assert.equal(packet.approval_pending_gate_count, 3);
...
test("removes a gate from approval packet after it has named approval", () => {
  ...
  const gate = approved.gates.MED250_GATE_DOMAIN_DNS_VERIFIED;
  gate.status = "confirmed";
  gate.approved_by = "Named infrastructure owner";
  gate.approved_role = "Infrastructure owner";
  gate.approved_at = "2026-07-20T18:00:00+02:00";

  const packet = buildLaunchApprovalPacket(approved);
  assert.equal(packet.approval_pending_gate_count, 2);
  assert.ok(!packet.gates.some(({ gate }) => gate === "MED250_GATE_DOMAIN_DNS_VERIFIED"));
});
