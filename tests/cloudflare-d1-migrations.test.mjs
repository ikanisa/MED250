...
test("applies every canonical migration to an isolated local D1 database", async (t) => {
  ...
  assert.deepEqual(files, [
    ...
    "0012_whatsapp_conversation_reliability.sql",
    "0013_partner_initial_request_permission.sql",
    "0014_partner_location_outreach.sql",
  ]);
  ...
  assert.equal(result[0].results[0].expected_migration, "0014_partner_location_outreach");
  assert.equal(result[0].results[0].expected_applied_count, 14);
