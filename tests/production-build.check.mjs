...
async function render(pathname) {
  ...
  return worker.fetch(new Request(`https://med250.rw${pathname}`, {
...
test("builds the explicit production Worker and custom-domain routes", async () => {
  ...
  assert.equal(config.name, "med250-marketplace");
...
test("publishes live robots directives and the complete source-backed sitemap", async () => {
  ...
  assert.match(robots, /Sitemap: https:\/\/med250\.rw\/sitemap\.xml/i);

  const sitemapResponse = await render("/sitemap.xml");
  assert.equal(sitemapResponse.status, 200);
  const sitemap = await sitemapResponse.text();
  assert.ok((sitemap.match(/<url>/g) ?? []).length >= 2_400);
  assert.match(sitemap, /https:\/\/med250\.rw\/product\/rwanda-fda-hm-/);
});
