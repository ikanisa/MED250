...
Generate one owner-ready JSON handoff containing every currently missing evidence type, its prepared pending artifact, current SHA-256 digest, byte length, unresolved checks, completion instructions, acceptance criterion and approval fields with:

```sh
npm run --silent launch:evidence:handoff
```

The handoff is a completion aid, not evidence. Prepared pending artifacts must not be referenced by the launch registry until they are genuinely complete, still match the reviewer-visible digest, and pass strict artifact validation. A generic template is generated only when no prepared packet exists.
