...
- The canonical Cloudflare Worker origin is `https://med-250.com`. The former `https://med250.gikundiro.com` origin passed all ten required routes on 2026-07-20 against revision `37d8c1c0e0c8ac2d15eea436d2f9037c20e2814c`; its registry artifacts are retained only as historical evidence and do not verify the replacement domain. Rerun the live verifier against the current release SHA before infrastructure-owner approval.
...
- The redacted pending account record is `docs/launch/evidence/cloudflare-account-verification-pending-2026-07-16.json`; it must remain unreferenced by the production registry. After the replacement credential is verified, run `npm run infra:cloudflare-account:evidence:build` from a redacted aggregate verification result to create completed account-verification and approval artifacts.
...
| `MED250_GATE_SECURITY_HARDENING_DEPLOYED` | Backend owner | Deployment receipt + test record | Review the current exact-revision Cloudflare deployment receipt, D1 migration readback, secret-binding inventory, aggregate operational snapshot and complete regression suite; then record the real backend-owner approval with `npm run launch:gate:approve -- --gate MED250_GATE_SECURITY_HARDENING_DEPLOYED --approved-by "Named backend owner" --approved-role "Backend owner" --approved-at "YYYY-MM-DDTHH:mm:ss+02:00"`. Historical Supabase artifacts are supporting history only. |
| `MED250_GATE_EDGE_FUNCTIONS_DEPLOYED` | Backend owner | Deployment receipt + test record | This legacy gate key now governs the single production Cloudflare Worker backend. Review fresh Worker deployment and test artifacts covering D1, R2, Queue, scheduled cleanup, Twilio inbound/status callbacks, authentication and protected review routes; then record the real backend-owner approval. Regenerate and bind fresh artifacts whenever the Worker Git revision changes. |
| `MED250_GATE_TURNSTILE_SERVER_VERIFIED` | Security owner | Test record | Use the real production widget to create one disposable Cloudflare Worker client session without sending an availability request; prove missing and invalid tokens are rejected, revoke the session, confirm no marketplace side effect, and retain only the redacted aggregate verifier result before generating the test artifact. |
| `MED250_GATE_AUTH_RATE_LIMITS_APPROVED` | Security owner | Signed approval + test record | Complete controlled Worker/D1 rate-limit tests for intended access, replay and excess-attempt rejection; remove disposable sessions, approve monitoring and rollback conditions, and generate the completed test and signed approval artifacts without phone numbers, tokens or row identifiers. |
...
| `MED250_GATE_CLOUDFLARE_ACCOUNT_VERIFIED` | Infrastructure owner | Account verification + signed approval | Create a deploy credential limited to the MED+250 production Worker, D1, Queue, R2/assets and direct custom-domain route with read-only zone inspection; remove broad release-path access, verify all nine required secret bindings and rollback authority, then generate redacted account-verification and signed approval artifacts. No staging or preview environment is part of the active release path. |
| `MED250_GATE_DOMAIN_DNS_VERIFIED` | Infrastructure owner | Domain verification + test record | Run `npm run domain:dns:verify`, then run `npm run deployment:verify -- --url https://med-250.com --mode live --expected-revision <current-sha> --evidence-output <receipt.json>`. If the receipt passes, run `npm run domain:evidence:refresh -- --deployment-evidence <receipt.json> --expected-revision git --date YYYY-MM-DD` to update the two domain artifacts and registry digests without approving the gate. Confirm DNS, TLS, routing, headers, robots and sitemap, then record named infrastructure-owner approval with `npm run launch:gate:approve -- --gate MED250_GATE_DOMAIN_DNS_VERIFIED --approved-by "Named infrastructure owner" --approved-role "Infrastructure owner" --approved-at "YYYY-MM-DDTHH:mm:ss+02:00"`. |
...
`med-250.com` is owned by the direct Cloudflare Worker route declared in `wrangler.jsonc`. Do not attach the same hostname to a Sites project or another Worker. The retired `med250.gikundiro.com` Worker hostname and `med250.rw` Sites plan are retained only as historical evidence and are not part of the active production verification path.
