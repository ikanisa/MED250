...
| --- | --- | --- |
| 0 — Live baseline | The [2026-07-18 live baseline](audit/live-baseline-2026-07-18.md) records populated audited departments, browser pagination to 168 settled cards, and a durable 10-route production receipt whose exact expected release value matched. Protected deploy CI injects a Git revision; the Worker/verifier enforce its response header, and the verifier can now require an exact expected value and atomically retain body-free route evidence. | Reconcile the observed manual release value to an immutable commit or governed deployment receipt, then bind fresh desktop/mobile and request-journey evidence to that source-traceable revision, reconcile live counts, and obtain approval. |
| 1 — Departments and full browsing | Zero-count departments are hidden from source-backed taxonomy; live RPC loading is deterministic, extends beyond the first 24, includes an accessible manual `Load more products` control, and exposes an explicit retry. The [source-bound live catalogue report](audit/live-catalogue-readiness-2026-07-18.md) proves all 39 pages, product 25, product 120, the final row, zero duplicate IDs, and non-zero populations in all four advertised departments. It also proves production still contains two retired records and fails both required multilingual aliases; the local migration repairs the alias contract. | Apply both pending catalogue migrations, obtain an exact 4,657-ID passing live receipt, then capture controlled failure recovery and complete desktop/mobile search and final-page evidence against the immutable release. |
...
| 8 — Product comprehension | Category-aware visual/JSON-LD breadcrumbs, URL-preserved catalogue state/focus, bounded customer titles, and exact official-name disclosure are implemented locally. A source-digest-bound 72-entry owner packet covers all 40 duplicate-title groups, 24 missing medicine generics, and 8 short-title candidates with strict fail-closed validation and no inferred clinical decisions. An atomic, locked, one-entry owner workflow validates the complete packet before every decision. Public descriptions now fail closed behind exact source digests, verified reuse rights, clinical applicability, substantive rationale, named review, and withdrawal guards. A protected single-product workflow computes the source digest from exact local bytes, uses optimistic concurrency, writes an immutable approval/withdrawal event, and prevents direct publication bypass. A new non-mutating verifier binds an anonymous-denial probe and one authenticated inspection to backend contract `2026-07-18.3`, the deployed reviewer contract, and an exact product version while retaining no response body or draft content. The related rail uses conservative medicine/consumer boundaries; the [live catalogue report](audit/live-catalogue-readiness-2026-07-18.md) now proves the same two suppressed non-product records are still present in production. | Deploy the description migrations and reviewer, capture the body-free reviewer receipt, complete and approve the 72 packet decisions, import authoritative corrections, deploy the existing retirement migration for the two proven live non-products, use the governed workflow to approve source-bound descriptions, reconcile live imagery, and capture browser evidence against live data. |
...
- [`docs/audit/browser-evidence-closure-2026-07-18.md`](audit/browser-evidence-closure-2026-07-18.md) defines the fail-closed capture and approval contract; its committed ledger stays pending until the controlled production run.
