...
interface Env {
  ASSETS: Fetcher;
  MED250_RELEASE_MODE?: "preview" | "live";
  IMAGES: {
...
const contentSecurityPolicy = [
  "default-src 'self'",
  "base-uri 'self'",
  "connect-src 'self' https://uskfnszcdqpcfrhjxitl.supabase.co wss://uskfnszcdqpcfrhjxitl.supabase.co",
  "font-src 'self' data:",
  "form-action 'self'",
...
function withSecurityHeaders(request: Request, response: Response, requestId: string, durationMs: number, releaseMode: Env["MED250_RELEASE_MODE"]) {
  ...
  if (releaseMode !== "live" || hostname.endsWith(".workers.dev")) headers.set("X-Robots-Tag", "noindex, nofollow");
