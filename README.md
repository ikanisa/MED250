...
The regulated medicine source remains the Rwanda FDA human-medicine register. Separately, the MED+250 consumer catalogue now contains 2,200 Amazon-first candidates across all 25 requested category/subcategory pairs, with at least 80 products in every pair. Those consumer rows are loaded in a separate Supabase table and remain inactive and non-orderable until seller verification and Rwanda compliance evidence are approved. The consolidated 4,680-record workbook and repeatable import/review procedure are documented in [`docs/marketplace-product-import.md`](docs/marketplace-product-import.md).
...
For an authorised release database, `npm run backend:verify` executes the service-only aggregate deployment contract. Version `2026-07-15.2` fails on any unreviewed change to the 28-function MED+250 API surface or 21-table GraphQL/RLS surface. In addition to the existing function, table, RLS, Realtime, GPS, contact-review, member-contact, and product-moderation allowlists, it proves atomic service-only OTP issuance, version-bound geocode approval, automatic contact-authority retirement, product-state revalidation at offer write and selection, database order-rate limiting, complete-offer filtering, and complete-offer contact release. Supply the service credential through the process environment only; the command never needs it in a file and the contract returns no row identifiers or object names.
