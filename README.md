...
- PostGIS `ST_DWithin` and distance ordering prioritise eligible pharmacies within 10 km, followed by a stable verified national responder fallback, with at most 10 recipients in total.
- Dispatch requires approved marketplace status, an active record, an unexpired premises entry, and a source- or admin-verified business WhatsApp contact. Portal access is separate: only an admin-approved, login-enabled WhatsApp number can request a pharmacy OTP. Approved GPS coordinates are required only for a pharmacy to be ranked or described as nearby. New orders are assigned to at most the 10 closest eligible pharmacies, with a deterministic verified-WhatsApp fallback where GPS is unavailable. All source-imported pharmacies are marketplace-approved automatically; approval is not displayed as a customer-facing badge.
...
6. Every source-imported pharmacy is marketplace-approved automatically. Source- or admin-verified WhatsApp contacts may receive order dispatches; this does not grant portal access. A pharmacy asks the MED+250 administrator to add a login-enabled WhatsApp contact before its staff can request a portal OTP. Separately verify current licences, staff authority, MoMo details, and precise premises coordinates before representing a pharmacy as nearby. A pharmacy without approved coordinates may participate through the verified national responder fallback when all other eligibility conditions pass.
...
`npm run release:check` runs the complete preview-safe gate in one command. Its dependency gate audits the complete Node tree at moderate severity or higher and checks every exact Python requirement pin against OSV. The image pipeline requires Python 3.11+ because supported Pillow and rembg releases no longer support the macOS system Python. Before a direct production redeployment, `npm run release:preflight:live` additionally requires live marketplace mode, an explicit HTTPS site origin, the public Turnstile configuration, and cloud observability. It never prints key values. The protected backend and operational-health checks require process-only production credentials; the Cloudflare packaging check remains a strict dry run before deployment.

`npm run launch:go-live:status` reports two separate outcomes. `siteProductionReady` covers the deployed site, domain and machine-verifiable runtime contract. `marketplaceTransactionReady` requires the controlled production Turnstile positive path and owner-observed physical-device transaction UAT. Missing source retention, duplicate-register decisions, product-content reviews and superseded approval bookkeeping remain non-blocking while the affected records stay quarantined, hidden or unpublished. `npm run launch:closure:board` lists those items as fail-closed follow-up and reserves its blocker list for genuine transaction evidence.
...
The operations command returns aggregate data only and never prints its credential. Dispatch coverage counts source- and admin-verified WhatsApp destinations independently from the smaller set of admin-approved portal-login contacts. `npm run ops:readiness:packet` generates a privacy-safe public-registry review index without phone numbers or coordinates. Products and indicative prices belong to the central catalogue. MED+250 publishes neither pharmacy-specific prices nor pharmacy stock; pharmacies privately confirm availability and the final interaction happens on WhatsApp. Prescription cleanup is installed through a Vault-backed six-hour schedule.
