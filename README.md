...
MED+250 is a product-first Rwanda pharmacy marketplace. A customer browses registered products, adds what they need to one order basket, shares a delivery location, and places the order. The order is dispatched to at most 20 nearby marketplace-approved pharmacies within 10 km. Customers see only pharmacies that confirm the complete order, ranked by distance and total price, then continue through WhatsApp. Contact details, exact coordinates, and prescription files stay private until a pharmacy responds and the customer chooses it.
...
The application defaults to `NEXT_PUBLIC_MARKETPLACE_MODE=preview`. Preview mode is intentional: it never fabricates order success or live pharmacy responses and it does not send customer health data. In live mode the customer journey stays deliberately short: add products, view basket, share location, place order, and review confirmed pharmacies.
...
The public fallback CSVs contain only official register fields. Historical staff phone lists and stale Google Maps candidates are not published or imported. Products with valid or expiring-soon registrations are available to add to an order; grace-period and expired records can remain visible for source transparency but are never orderable. Prescription handling is conservative until an authorised classification review is completed.
...
- Live catalogue browsing uses the paginated `dawanear_search_catalogue` Supabase RPC instead of downloading every product into the customer browser. It ranks exact names and ingredients, full-text matches, common-use English/French/Kinyarwanda aliases, and close spelling matches; category, prescription, dosage-form, availability, sort, price-range aggregation, and stable result counts are applied server-side. The in-browser scorer remains only as the preview/offline fallback. The RPC is `SECURITY INVOKER`, has an empty search path, exposes no pharmacy identity, and is executable only by `anon` and `authenticated`.
- Supabase anonymous Auth for customers; six-digit WhatsApp Cloud API OTP for pharmacy staff. The portal has no email sign-up or claim path. A pharmacy number must already be attached to an active, marketplace-approved pharmacy record. Location dispatch additionally requires verified GPS coordinates. Successful OTP exchange creates a permanent Supabase session in a pharmacy-only browser store; it refreshes and persists until staff explicitly sign out.
...
- Dispatch requires approved marketplace status, verified GPS coordinates, an active record, and an unexpired premises entry. All source-imported pharmacies are marketplace-approved automatically; approval is not displayed as a customer-facing badge.
...
2. Keep the site in connected preview mode until authoritative pharmacy GPS, operational contacts, and the documented Rwanda marketplace operating approvals are complete.
...
`npm run release:check` runs the complete preview-safe gate in one command. Before a production cutover, `npm run release:preflight:live` additionally requires live marketplace mode and an explicit HTTPS site origin. It never prints key values. The final `npm run release:check:live` command then requires the deployed backend contract and strict operational-health snapshot to pass before lint, data validation, tests, budgets, and Cloudflare packaging can proceed. Keep `.env.local` in preview mode until the GPS, regulated-operation, authorised test-identity, and production-domain gates are complete.
...
For an authorised release database, `npm run backend:verify` executes the service-only aggregate deployment contract. Version `2026-07-14.1` fails on any unreviewed change to the 26-function MED+250 API surface or 19-table GraphQL/RLS surface. In addition to the existing function, table, RLS, Realtime, GPS, contact-review, and member-contact allowlists, it proves atomic service-only OTP issuance, version-bound geocode approval, automatic contact-authority retirement, product-state revalidation at offer write and selection, database order-rate limiting, complete-offer filtering, and complete-offer contact release. Supply the service credential through the process environment only; the command never needs it in a file and the contract returns no row identifiers or object names.
...
`npm run data:duplicates:packet` generates a deterministic private reviewer packet containing all 51 groups, source references, exact field values, differing fields and source-file SHA-256 digests. It contains no inferred decision or recommendation; the named regulatory reviewer must still inspect the authoritative record and write the decision only to the governed ledger.

The test suite also executes the real catalogue-search TypeScript module and the actual PostgreSQL search migration in PGlite. This covers exact ingredient ranking, multilingual aliases, alias-over-fuzzy precedence, typo recovery, category/form/availability filters, aggregate price ranges, and stable pagination totals without making a live database write. The storefront preserves the server RPC's result order in live mode; it does not re-rank a server page with the preview fallback scorer.
...
For an authorised release database, [`tests/live-marketplace-rollback.sql`](tests/live-marketplace-rollback.sql) executes the deployed order lifecycle inside one transaction and then rolls it back. It proves automatic retail-pharmacy marketplace approval, fail-closed GPS/WhatsApp eligibility, 10 km recipient dispatch, idempotent retry, membership isolation, complete confirmation, pre-selection contact privacy, customer ownership, selection/contact release, completion and notification lifecycle while leaving no customer, pharmacy, contact, order, offer or notification fixture behind. Run it through the Supabase SQL editor or an authorised database connection; never add a database password to the repository.
...
The repository includes a Cloudflare Worker entry point and `wrangler.jsonc`. The default configuration is the isolated `med250-marketplace-preview` Worker with `MED250_RELEASE_MODE=preview`; an explicit `production` environment targets `med250-marketplace`, disables `workers.dev`, routes `med250.rw` plus `www.med250.rw`, and sets `MED250_RELEASE_MODE=live`. This follows Cloudflare's named-environment model while keeping a routine/default deployment away from production traffic. Validate the preview bundle without publishing it:
...
Set `NEXT_PUBLIC_SITE_URL` to the final HTTPS origin and configure the public Supabase URL and publishable key as build variables. Keep service-role, WhatsApp Cloud API, Google Maps, admin-token, and other server credentials only in protected CI or Supabase Edge Function secrets. `npm run deployment:verify -- --url <https-url> --mode preview|live` can be rerun independently after any deployment. A successful dry run proves the Worker bundle and bindings are valid; it does not replace production domain, DNS, runtime, authentication, order-dispatch, or Core Web Vitals verification. On this machine `wrangler whoami` currently reports that no Cloudflare account is authenticated, so Worker existence, zone ownership, route attachment and protected variables must still be verified after an authorised login.

The owner-only Sites project now has pending custom-domain attachments for `med250.rw` and `www.med250.rw`. The zone still has no routing or validation records, so no custom-domain traffic is active. The exact provider-issued plan is in [`docs/launch/dns/med250-sites-domain-plan.json`](docs/launch/dns/med250-sites-domain-plan.json); `npm run domain:dns:verify` compares public DNS with that plan and intentionally exits non-zero until every required record is visible. DNS agreement alone never confirms the launch gate. Sites provider/TLS activation, authenticated route verification, the routing-owner decision and named infrastructure approval are also required. Do not deploy the direct Wrangler production routes while the same hostnames remain attached to Sites.
