...
function mapPublicMarketplaceProduct(row: CatalogueRow, fallbackId: string): Product {
  ...
  return {
    ...
    verifiedOfferCount: Math.max(0, Math.round(number(row, "verified_offer_count"))),
    verifiedOfferMinRwf: Math.max(0, Math.round(number(row, "verified_offer_min_rwf"))),
    verifiedOfferMaxRwf: Math.max(0, Math.round(number(row, "verified_offer_max_rwf"))),
    verifiedOfferUpdatedAt: text(row, "verified_offer_updated_at") || null,
