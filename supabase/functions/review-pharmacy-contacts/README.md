...
Use `npm run ops:contacts -- list`, inspect a request, verify the contact directly with the pharmacy, then approve or reject it with a named reviewer and evidence note. Approved WhatsApp numbers become login-enabled and are mirrored into the pharmacy's phone contacts by the database transaction. No public phone listing is automatically promoted to WhatsApp.
